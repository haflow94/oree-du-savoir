...
| `Document` (`documents`) | `etudiantId` (FK), `type` (enum `TypeDocument`), `nomFichier`, `cheminRelatif`, `mimeType`, `tailleOctets`, `creeParId` (FK optionnelle) | PK `id` ; FK `etudiant` Cascade, `creePar` `SetNull` ; index `etudiantId` | Métadonnées uniquement — le fichier réel vit sous `DOCUMENTS_DIR`, jamais en base (règle non négociable). `mimeType` est désormais toujours dérivé du contenu réel du fichier à l'upload (jamais du `type` déclaré par le navigateur) pour le champ public photo — voir `src/lib/fichiers-uploades.ts` et §6.5. La pièce d'identité (`type = PIECE_IDENTITE`) n'est plus collectée depuis le 2026-09-16 ; l'enum est conservée pour l'historique et pour la pièce d'identité du titulaire d'un chèque tiers (`chequeId`, voir `paiements/[id]/actions.ts`). |
| `CodePreinscription` (`codes_preinscription`) | `codeHash` (unique, empreinte SHA-256), `campagneLabel`, `expireLe`, `estAccueilAuto`, `usageUnique`, `utiliseLe` (nullable), `creeParId` (FK optionnelle vers `Utilisateur`) | PK `id` ; FK `creePar` `SetNull` ; index `expireLe` | Code d'accès pour `/preinscription` (voir `src/lib/preinscription-code.ts` et §6.5) — jamais un verrou global : n'entre en jeu que si l'URL porte `?code=...`. Deux origines (`estAccueilAuto`) : code email à usage unique généré à la main, ou code accueil auto-régénéré (`usageUnique = false`, expire en fin de journée). |
...
| **Inscriptions** | `(app)/inscriptions/` | File d'attente des préinscriptions publiques (`statutInscription = PREINSCRIT`) en attente de contrôle physique avant validation. Module de permission distinct d'Étudiants. Porte aussi, pour qui a `ECRITURE` sur ce module, le lien `/accueil-preinscription` (QR stable affiché à l'accueil, redirige vers un code auto-régénéré) avec un bouton pour forcer sa rotation anticipée, et la génération d'un **code de préinscription à usage unique** pour une campagne email ciblée (`generer-code-form.tsx`/`invalider-code-accueil-button.tsx`/`actions.ts` — voir §6.5). |
...
| **Préinscription publique** | `src/app/preinscription/` (hors layout `(app)`) | Formulaire public sans authentification. Supporte **plusieurs cours/sections en une seule soumission** (une ligne section + créneau par cours souhaité, ajoutables dynamiquement) ; crée directement les `InscriptionClasse` correspondantes si un créneau est ouvert, sinon mémorise la première section sans créneau sur `sectionSouhaiteeId` (et note les suivantes en `remarque`) ; lance la détection de doublon automatique sans jamais bloquer la soumission. Durci pour une exposition Internet (voir §6.5) : validation du contenu réel des fichiers joints, limite de taille, code d'accès optionnel (`?code=...`, généré depuis *Inscriptions* ou via le QR d'accueil), rate limiting applicatif. |
...
Sur le champ public photo (`preinscription/actions.ts`) : contenu vérifié par signature binaire (whitelist stricte PDF/JPEG/PNG, `src/lib/fichiers-uploades.ts`) et plafonné à `TAILLE_MAX_FICHIER_MO` (8 Mo par défaut, `.env`) — jamais l'extension ou le `type` déclaré par le navigateur. Toute route qui sert un document recalcule de même son `Content-Type`/`Content-Disposition` à partir du contenu réel (`src/lib/service-fichier.ts`) : un fichier hors whitelist est toujours forcé en téléchargement (`attachment`), jamais affiché `inline` avec un type non vérifié. Détail complet en §6.5.
...
| `GET /dossiers-signes-a-notifier` + `POST /dossiers-annuels/[dossierAnnuelId]/notification-signature-confirmee` | `DossierAnnuel` | `notificationSignatureEnvoyeeLe` | Email famille : signature Documenso confirmée (dès `statutSignature = SIGNEE`), avec le PDF signé (`Document` type `DOSSIER_SIGNE`) en pièce jointe. Le `POST` est scopé par `dossierAnnuelId` (fourni par chaque candidat du `GET`), pas par `etudiantId` : un étudiant réinscrit peut avoir plusieurs dossiers annuels `SIGNEE` non notifiés simultanément, et scoper par étudiant obligeait à redeviner "le plus ancien non notifié" côté serveur — non atomique avec le marquage, donc sujet à une race condition entre deux appels n8n concurrents (voir git history). |
| `GET /dossiers-a-relancer` + `POST /dossiers-annuels/[dossierAnnuelId]/relance-envoyee` | `DossierAnnuel` | `nombreRelancesEnvoyees` + `derniereRelanceEnvoyeeLe` (compteur, pas un simple booléen) | Relance paiement (aucun paiement apporté), plafonnée par `ParametresRelance.nombreMaxRelances`. |
...
| `GET /documents/[documentId]/fichier` | — | — | Sert le contenu d'un PDF (uniquement type `DOSSIER_GENERE` ou `DOSSIER_SIGNE`) référencé par un email n8n — c'est cette même route qui sert la pièce jointe du flux `/dossiers-signes-a-notifier` ci-dessus. |
...
### 6.5 Sécurisation de la préinscription publique

Correctifs applicatifs préparant `/preinscription` à une exposition Internet filtrée par un tunnel sortant + anti-abus au niveau réseau (voir `ARCHITECTURE-PREINSCRIPTION.md`/`DEPLOIEMENT-PREINSCRIPTION.md` — la couche réseau est en place : tunnel Cloudflare + Managed Challenge sur `preinscription.loreedusavoir.fr`, déployé le 2026-09-16 ; seul le filtrage de route fin côté fournisseur reste à faire). Défense en profondeur : ces correctifs restent nécessaires même sans exposition externe, et ne comptent jamais uniquement sur le filtrage réseau.

- **Validation de fichiers** (`src/lib/fichiers-uploades.ts`, sans nouvelle dépendance — signature binaire vérifiée à la main) : whitelist stricte PDF/JPEG/PNG sur `photo`/`pieceIdentite`, taille plafonnée par `TAILLE_MAX_FICHIER_MO` (`.env`, défaut 8 Mo, répercuté dans `next.config.ts` sur `experimental.serverActions.bodySizeLimit` — sans quoi la limite globale par défaut de Next, 1 Mo, n'est pas une protection volontaire). Validé **avant** toute écriture en base (`preinscription/actions.ts`) pour ne jamais laisser un étudiant préinscrit orphelin d'un document attendu.
- **Service de document durci** (`src/lib/service-fichier.ts`) : `Content-Type` recalculé depuis le contenu réel, jamais depuis `Document.mimeType` (déclaré au moment de l'upload, pas fiable) ; `Content-Disposition: inline` réservé aux trois types sûrs, tout le reste forcé en `attachment`. Appliqué à `etudiants/[id]/documents/[documentId]/route.ts` et `administration/gouvernance/documents/[documentId]/route.ts`.
- **Code d'accès** (`CodePreinscription`, `src/lib/preinscription-code.ts`) : n'est un verrou que si `?code=...` est présent dans l'URL — son absence conserve le comportement existant (accès direct sans code), décision explicite pour ne pas casser silencieusement un flux déjà en production. Seul le hash SHA-256 est stocké (`hashSessionToken`, réutilisé tel quel). Deux origines, distinguées par `estAccueilAuto`, avec un mode de consommation différent (`usageUnique`) : un code **email** (`estAccueilAuto = false`, `usageUnique = true`), généré à la main depuis *Inscriptions* (rôle `ECRITURE` sur `INSCRIPTIONS`) pour une campagne ciblée, consommé de façon atomique (`updateMany` conditionnel sur `utiliseLe: null`) juste avant la création de l'étudiant ; un code **accueil** (`estAccueilAuto = true`, `usageUnique = false`), minté automatiquement par `GET /accueil-preinscription` qui redirige vers `/preinscription?code=...` — lien stable imprimé une seule fois à l'accueil, réutilisant le code en cours tant qu'il est valide et n'expirant qu'à la fin de la journée courante (`finDeJourneeCourante`, `TZ` du conteneur) plutôt qu'à la soumission, pour que plusieurs familles scannant le même QR le même jour puissent toutes soumettre. Le staff peut forcer une rotation anticipée (fuite suspectée) depuis *Inscriptions* (`invaliderCodeAccueilAutoEnCours`).
- **Rate limiting applicatif** (`src/lib/rate-limit.ts`) : sliding window en mémoire (pas de Redis/table dédiée pour ce volume), sur la vérification de code (`preinscription/page.tsx`) et la soumission (`preinscrireAction`). Complément — jamais un remplacement — de l'anti-abus au niveau du point d'entrée public. Réinitialisé à chaque redémarrage du conteneur (acceptable pour une protection de second rang).
- **Anti-abus réseau** : règle de sécurité Cloudflare "Anti-abus préinscription" (WAF → Règles de sécurité → Règles personnalisées, action Managed Challenge/Turnstile) sur le hostname `preinscription.loreedusavoir.fr` — entièrement hors du dépôt de code, configurée dans le tableau de bord Cloudflare du domaine (jamais dans Zero Trust). Déployée et vérifiée le 2026-09-16 (voir `ARCHITECTURE-PREINSCRIPTION.md`).
- **Réseau** (`docker-compose.yml`) : ports `5432` (Postgres) et `5678` (n8n) liés à `127.0.0.1` plutôt que publiés sur toutes les interfaces — inutile de les exposer au réseau local pour que l'app fonctionne (elle les joint via le réseau interne Docker), et strictement aucun rapport avec la préinscription publique.
- **Cookie de session** (`src/lib/auth.ts`) : la logique existante (`secure` conditionné à `x-forwarded-proto`, pas à `NODE_ENV`) reste correcte telle quelle une fois un tunnel public HTTPS en place, à condition que ce tunnel définisse fidèlement cet en-tête et qu'aucun client externe ne puisse l'atteindre directement sans passer par lui — voir le commentaire détaillé dans le code plutôt qu'un changement de comportement.
