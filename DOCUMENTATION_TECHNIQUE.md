...
| `Classe` (`classes`) | `coursId` (FK), `anneeScolaireId` (FK), `niveau`, `semestre`, `jour` (enum `JourSemaine`), `heureDebut`/`heureFin` (`"HH:MM"`), `salle`, `capacite`, `qrToken` (unique, cuid) | PK `id` ; FK `Restrict` sur `cours`/`anneeScolaire` ; index `anneeScolaireId`, `coursId` | Groupe concret planifié : un seul créneau hebdomadaire par classe, plusieurs enseignants possibles (via `ClasseEnseignant`). `qrToken` est l'identifiant public affiché en salle (jamais une authentification). |
...
| **Classes** | `(app)/classes/` | Référentiel Cours (matière) et Classes (groupe planifié : créneau, salle, capacité, enseignants), duplication d'une année sur l'autre, **garde-fou anti-doublon** (cours + niveau + année + semestre) à la création et à la modification. |
| **Présences** | `(app)/presences/`, `(app)/appel/[seanceId]/`, `(app)/qr/[token]/` | Génération des séances depuis le planning, feuille d'appel (statuts P/R/RE/A/AE), validation par l'enseignant, correction dans un délai limité (sauf staff administratif), annulation de séance, gestion des périodes de fermeture. Accès QR scanné en salle → session restreinte à la séance du jour. |
...
| **Documents** | transverse (`(app)/etudiants/[id]/dossier/`, `[id]/documents/[documentId]/`) | Génération du dossier d'inscription officiel au format **.docx** (gabarits Word réels de l'association, remplis via `docxtemplater`/`pizzip` — voir §3.4), téléversement/consultation/suppression des pièces jointes d'un étudiant. Module de permission séparé d'Étudiants (une même personne peut avoir accès aux fiches sans pouvoir gérer les documents, ou l'inverse). |
...
### 3.4 Génération du dossier officiel

⚠️ Ce point diverge de `CLAUDE.md` actuel (voir §6) : la génération ne produit **pas** un PDF via `@react-pdf/renderer`, mais un document **Word (.docx)** en remplissant directement les gabarits `.docx` d'origine de l'association (un gabarit par Section, balises `{tag}` insérées à la main) via `docxtemplater` + `pizzip` (`src/lib/dossier-officiel.ts`, fichier `.ts`, pas `.tsx`). Le handler `(app)/etudiants/[id]/dossier/route.ts` (module `DOCUMENTS`, niveau `ECRITURE`) assemble les données live (étudiant, responsable, créneaux inscrits, tarifs/barème de la Section choisie), appelle la génération, persiste le résultat comme `Document` (type `DOSSIER_GENERE`) pour qu'il reste consultable après la signature physique, puis le streame en téléchargement (`Content-Disposition: attachment`, un navigateur ne prévisualisant pas un `.docx`).
...
| `docxtemplater` + `pizzip` | ^3.69.3 / ^3.2.0 | Remplissage des gabarits `.docx` du dossier officiel (voir §3.4). |
...
- **`CLAUDE.md` est obsolète sur la génération du dossier officiel** : il décrit un fichier `src/lib/dossier-officiel.tsx` utilisant `@react-pdf/renderer` pour produire un PDF. Le code réel est `src/lib/dossier-officiel.ts` (`.ts`, pas `.tsx`) et produit un `.docx` via `docxtemplater`/`pizzip` en remplissant les gabarits Word d'origine de l'association — `@react-pdf/renderer` n'apparaît nulle part dans `package.json`. À corriger dans `CLAUDE.md` lors d'une prochaine passe.
