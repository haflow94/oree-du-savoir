...
/**
 * Cible du QR permanent affiché en salle (voir Salle.qrToken) : un seul code
 * fixe par salle, jamais changé entre les cours qui s'y succèdent. Le QR est
 * un raccourci d'accès, jamais une authentification (règle non négociable) :
 * sans session valide, on renvoie vers /login, et l'accès à la classe est
 * vérifié comme partout ailleurs. Une fois la séance trouvée, direction
 * /appel/{id} — une page isolée, sans aucun lien vers le reste de l'appli
 * (voir src/app/appel/[seanceId]).
 */
export default async function QrPage({
  ...
  const resolution = await resoudreSeanceDuJourPourSalle(token);
  ...
  if (resolution.raison === "TOKEN_INCONNU") {
    return (
      <Message titre="QR code inconnu">
        Ce QR code ne correspond à aucune salle. Il a peut-être été régénéré.
      </Message>
    );
  }
  ...
  if (resolution.raison === "PAS_DE_SEANCE") {
    return (
      <Message titre={`${resolution.salleNom} — pas de séance aujourd'hui`}>
    ...
    <div className="flex min-h-screen flex-1 items-center justify-center bg-bg px-4">
      <div className="w-full max-w-sm rounded-xl border border-border bg-bg-elevated p-6 text-center shadow-card">
        <h1 className="font-display text-base font-semibold text-pine-strong">
          {resolution.salleNom} — plusieurs cours aujourd&apos;hui
        </h1>
