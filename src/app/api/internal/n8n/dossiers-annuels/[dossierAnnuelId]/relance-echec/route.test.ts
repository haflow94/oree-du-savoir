...
function requete(token: string | undefined, body?: unknown): NextRequest {
  ...
  return new NextRequest(
    "http://localhost/api/internal/n8n/dossiers-annuels/dos1/relance-echec",
...
describe("POST /api/internal/n8n/dossiers-annuels/[dossierAnnuelId]/relance-echec", () => {
  ...
  it("enregistre l'échec sans toucher au compteur de relances réussies", async () => {
    ...
    expect(updateMany).toHaveBeenCalledWith({
      where: { id: "dos1" },
      data: {
        derniereRelanceErreurLe: expect.any(Date),
        derniereRelanceErreurMessage: "550 mailbox unavailable",
      },
    });
  });
