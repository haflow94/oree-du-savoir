...
function requete(token?: string): NextRequest {
  ...
  return new NextRequest("http://localhost/api/internal/n8n/dossiers-a-verifier/dos1/notifie", {
    method: "POST",
    headers,
  });
}
...
describe("POST /api/internal/n8n/dossiers-a-verifier/[dossierAnnuelId]/notifie", () => {
  ...
  it("marque le dossier notifié la première fois (idempotence — scénario nominal)", async () => {
    updateMany.mockResolvedValue({ count: 1 });

    const reponse = await POST(requete(SECRET), params());
  ...
  it("scénario 9 — un second appel sur le même dossier (retry n8n) ne provoque aucune erreur ni double marquage", async () => {
