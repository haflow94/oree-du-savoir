...
- `Organisation` is a single row holding the association's identity (name, address, RNA, logo) used by generated documents — read via `getOrganisation()` (`src/lib/organisation.ts`). No `organisationId` anywhere else yet: the app stays single-tenant on purpose (see git history for the multi-tenant readiness analysis if that ever needs revisiting).
