...
- `Section` (Jeunes, Langue Arabe, Études Coraniques, Études Islamiques) is the pricing/refund-schedule reference data, editable from *Administration → Sections*, not hardcoded. A `Cours` belongs to a `Section`. `Section.modeleDossier` (ADULTES/JEUNES) selects which of the 2 master dossier templates applies; `CreneauSection` is the catalogue of time-slot cards shown on that section's dossier (distinct from a `Classe`'s actual weekly slot).
- `Organisation` is a single row holding the association's identity (name, address, SIRET/NAF, logo) used by generated documents — read via `getOrganisation()` (`src/lib/organisation.ts`). No `organisationId` anywhere else yet: the app stays single-tenant on purpose (see git history for the multi-tenant readiness analysis if that ever needs revisiting).
...
- `Document` stores metadata only; the actual file lives on disk under `DOCUMENTS_DIR` (`src/lib/documents.ts`). Never build a file path from client input — always resolve through the DB row's stored `cheminRelatif`. `mimeType` on public-facing uploads (préinscription's `photo`/`pieceIdentite`) is always the content-sniffed type (`src/lib/fichiers-uploades.ts`, whitelist PDF/JPEG/PNG via magic bytes), never the browser-declared `File.type`. Every document-serving route recomputes `Content-Type`/`Content-Disposition` from the actual bytes at serve time too (`src/lib/service-fichier.ts`) — only that whitelist may be served `inline`, everything else is forced to `attachment` regardless of the stored `mimeType`.
- `CodePreinscription` is an access code gating `/preinscription` only when the URL carries `?code=...` (direct URL access without a code is left untouched) — only its SHA-256 hash is stored (same `hashSessionToken` as `Session`), checked/consumed via `src/lib/preinscription-code.ts`. Two origins, distinguished by `estAccueilAuto`, with a different consumption mode (`usageUnique`): an **email** code (`estAccueilAuto = false`, `usageUnique = true`) is generated by hand from *Inscriptions* (module `INSCRIPTIONS`, ECRITURE) for one targeted campaign, consumed atomically on the first successful submission (`utiliseLe`); an **accueil** code (`estAccueilAuto = true`, `usageUnique = false`) is minted automatically by `GET /accueil-preinscription` (`src/app/accueil-preinscription/route.ts`) — the stable link printed once at reception — reuses the current still-valid code across scans and expires at the end of the current calendar day (`finDeJourneeCourante`, server `TZ`), never consumed by a submission so several families can use the same printed QR the same day; staff can also force an early rotation (suspected leak) via *Inscriptions* → `invaliderCodeAccueilAutoEnCours`.
...
### Dossier d'inscription generation (`src/lib/dossier/`)

Exactly 2 master templates — `templates/adultes.hbs` and `templates/jeunes.hbs` (Handlebars) — never one per `Section`: which one applies is `Section.modeleDossier`. `context.ts` builds the render context from live DB data (`Organisation`, `Section` + its `CreneauSection` catalogue, étudiant, responsable légal, actual inscription) with the same shape for a real étudiant or a blank ("vierge") dossier — an empty field just renders as a blank underline, matching the paper-form look. `render.ts` compiles the template and prints it to PDF via headless Chromium (`puppeteer-core`, singleton browser in `browser.ts`, driven by `PUPPETEER_EXECUTABLE_PATH` — the Dockerfile installs the system `chromium` package rather than Puppeteer's own download). Logo and student photo are inlined as `data:` URIs (`src/lib/organisation.ts`) and fonts are self-hosted under `public/fonts/`, so a PDF never depends on network access to render. One template drives both the on-screen preview and the PDF: "viewing" a dossier means opening this same generated PDF inline in the browser (native PDF viewer print/download controls), never a separate HTML render.

Route handler `(app)/etudiants/[id]/dossier/route.ts` (module DOCUMENTS, ECRITURE) renders it, persists it as a `Document` (type DOSSIER_GENERE, `application/pdf`) so it stays retrievable after the physical signature, and streams it back (`?dl=1` forces a download instead of the default inline preview). `(app)/documents/dossier-vierge/` is the same engine without an étudiant, never persisted as a `Document`.
...
`Projet/` holds the original functional spec — read in the order listed in `Projet/README_CLAUDE.md`. `DEPLOIEMENT.md` covers Docker deployment, backup/restore, and required env vars (`.env.example`). `ARCHITECTURE-PREINSCRIPTION.md`/`DEPLOIEMENT-PREINSCRIPTION.md` cover the (not-yet-deployed) public-internet exposure of `/preinscription` via an outbound tunnel + edge filtering — the application-side prerequisites they list are implemented (see `DOCUMENTATION_TECHNIQUE.md` §6.5), the network layer itself is not.
